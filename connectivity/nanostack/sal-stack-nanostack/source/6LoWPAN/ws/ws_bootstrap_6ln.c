...
static void ws_bootstrap_6ln_pan_information_store(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us, ws_pan_information_t *pan_information)
{
...
static void ws_bootstrap_6ln_pan_advertisement_analyse_active(struct protocol_interface_info_entry *cur, ws_pan_information_t *pan_information)
{
    /* In Active state
     *
...
static void ws_bootstrap_6ln_pan_advertisement_analyse(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, const struct mcps_data_ie_list *ie_ext, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us)
{
    ...
    ws_bootstrap_6ln_pan_information_store(cur, data, ws_utt, ws_us, &pan_information);
    ...
    ws_bootstrap_6ln_pan_advertisement_analyse_active(cur, &pan_information);
...
static void ws_bootstrap_6ln_start_configuration_learn(protocol_interface_info_entry_t *cur)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
...
static void ws_bootstrap_6ln_ip_stack_addr_clear(protocol_interface_info_entry_t *cur)
{
...
static void ws_bootstrap_6ln_start_discovery(protocol_interface_info_entry_t *cur)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
    ...
    ws_bootstrap_6ln_ip_stack_addr_clear(cur);
...
static int8_t ws_bootstrap_6ln_fhss_configure(protocol_interface_info_entry_t *cur, bool discovery)
{
...
void ws_bootstrap_6ln_network_discovery_configure(protocol_interface_info_entry_t *cur)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
    ...
    ws_bootstrap_6ln_fhss_configure(cur, true);
...
static struct rpl_instance *ws_bootstrap_6ln_get_rpl_instance(protocol_interface_info_entry_t *cur)
{
...
static uint16_t ws_bootstrap_6ln_rank_get(protocol_interface_info_entry_t *cur)
{
    struct rpl_instance *rpl_instance = ws_bootstrap_6ln_get_rpl_instance(cur);
    if (!rpl_instance)
    {
        return 0xffff;
    }
...
static uint16_t ws_bootstrap_6ln_min_rank_inc_get(protocol_interface_info_entry_t *cur)
{
    struct rpl_instance *rpl_instance = ws_bootstrap_6ln_get_rpl_instance(cur);
    if (!rpl_instance)
    {
...
static void ws_bootstrap_6ln_set_fhss_hop(protocol_interface_info_entry_t *cur)
{
    uint16_t own_rank = ws_bootstrap_6ln_rank_get(cur);
    uint16_t rank_inc = ws_bootstrap_6ln_min_rank_inc_get(cur);
...
static void ws_bootstrap_6ln_parent_confirm(protocol_interface_info_entry_t *cur, struct rpl_instance *instance)
{
...
static void ws_bootstrap_6ln_pan_config_lfn_analyze(struct protocol_interface_info_entry *cur, const struct mcps_data_ie_list *ie_ext)
{
...
static void ws_bootstrap_6ln_pan_config_analyse(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, const struct mcps_data_ie_list *ie_ext, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us)
{
...
#ifdef HAVE_WS_VERSION_1_1
            ws_bootstrap_6ln_pan_config_lfn_analyze(cur, ie_ext);
#endif
            return;
        }
        else
...
static void ws_bootstrap_6ln_pan_advertisement_solicit_analyse(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us)
{
    ...
    if (ws_bootstrap_state_active(cur) && cur->bootsrap_mode != ARM_NWK_BOOTSRAP_MODE_6LoWPAN_BORDER_ROUTER)
    {
        ...
        {
            ws_bootstrap_6ln_parent_confirm(cur, NULL);
        }
    }
}

static void ws_bootstrap_6ln_pan_config_solicit_analyse(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, ws_utt_ie_t *ws_utt, ws_us_ie_t *ws_us)
{
    if (data->SrcPANId != cur->ws_info->network_pan_id)
    {
        return;
...
void ws_bootstrap_6ln_asynch_ind(struct protocol_interface_info_entry *cur, const struct mcps_data_ind_s *data, const struct mcps_data_ie_list *ie_ext, uint8_t message_type)
{
    ...
    case WS_FT_PAN_ADVERT:
        ...
        ws_bootstrap_6ln_pan_advertisement_analyse(cur, data, ie_ext, &ws_utt, &ws_us);
        break;
    case WS_FT_PAN_ADVERT_SOL:
        ...
        ws_bootstrap_6ln_pan_advertisement_solicit_analyse(cur, data, &ws_utt, &ws_us);
        break;
    case WS_FT_PAN_CONF:
        ...
        ws_bootstrap_6ln_pan_config_analyse(cur, data, ie_ext, &ws_utt, &ws_us);
        break;
    case WS_FT_PAN_CONF_SOL:
        ...
        ws_bootstrap_6ln_pan_config_solicit_analyse(cur, data, &ws_utt, &ws_us);
    default:
...
static void ws_bootstrap_6ln_address_update(protocol_interface_info_entry_t *interface)
{
    tr_info("RPL parent update ... register ARO");
    ws_bootstrap_6ln_address_registration_update(interface, NULL);
}

static void ws_bootstrap_6ln_rpl_scan_start(protocol_interface_info_entry_t *cur)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
...
void ws_bootstrap_6ln_secondary_parent_update(protocol_interface_info_entry_t *interface)
{
    if (interface->ws_info)
    {
        ns_list_foreach(if_address_entry_t, address, &interface->ip_addresses)
        {
            if (!addr_is_ipv6_link_local(address->address))
            {
                ws_bootstrap_6ln_address_update(interface);
            }
        }
    }
}
...
static void ws_bootstrap_6ln_rpl_callback(rpl_event_t event, void *handle)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
    ...
    if (event == RPL_EVENT_DAO_DONE)
    {
        ...
        {
            /*if (cur->bootsrap_mode == ARM_NWK_BOOTSRAP_MODE_6LoWPAN_BORDER_ROUTER || cur->bootsrap_mode == ARM_NWK_BOOTSRAP_MODE_6LoWPAN_ROUTER)
            {
        ...
        ws_bootstrap_6ln_set_fhss_hop(cur);
    ...
    else if (event == RPL_EVENT_LOCAL_REPAIR_START)
    {
        ...
        if (ws_bootstrap_state_active(cur))
        {
            ...
            ws_bootstrap_6ln_rpl_scan_start(cur);
            ws_bootstrap_network_down(cur);
        }
    }
    else if (event == RPL_EVENT_DAO_PARENT_ADD)
    {
        ws_bootstrap_6ln_address_update(cur);
    }
    cur->ws_info->rpl_state = event;
    tr_info("RPL event %d", event);
}

static void ws_bootstrap_6ln_rpl_prefix_callback(prefix_entry_t *prefix, void *handle, uint8_t *parent_link_local)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
...
static bool ws_bootstrap_6ln_rpl_candidate_soft_filtering(protocol_interface_info_entry_t *cur, struct rpl_instance *instance)
{
...
static bool ws_bootstrap_6ln_rpl_new_parent_callback(uint8_t *ll_parent_address, void *handle, struct rpl_instance *instance, uint16_t candidate_rank)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
    protocol_interface_info_entry_t *cur = handle;
    if (!cur->rpl_domain || cur->interface_mode != INTERFACE_UP)
    ...
    if (!ws_bootstrap_6ln_rpl_candidate_soft_filtering(cur, instance))
    {
...
static void ws_bootstrap_6ln_rpl_parent_dis_callback(const uint8_t *ll_parent_address, void *handle, struct rpl_instance *instance)
{
    /// TODO port the below code which is part of router bootstrap for leaf node application
    ...
    ws_bootstrap_6ln_parent_confirm(cur, instance);
}
...
void ws_bootstrap_6ln_event_handler(protocol_interface_info_entry_t *cur, arm_event_s *event)
{
    ...
    case WS_DISCOVERY_START:
        ...
        ws_bootstrap_6ln_network_discovery_configure(cur);
        ...
        // Start network scan
        ws_bootstrap_6ln_start_discovery(cur);
        break;
    ...
    case WS_CONFIGURATION_START:
        ...
        ws_bootstrap_6ln_start_configuration_learn(cur);
        break;

    case WS_OPERATION_START:
        tr_info("Operation start");
        // Advertisements stopped during RPL scan
        ws_bootstrap_asynch_trickle_stop(cur);
        // Activate RPL
        // Activate IPv6 stack
        ...
        rpl_control_set_callback(protocol_6lowpan_rpl_domain, ws_bootstrap_6ln_rpl_callback, ws_bootstrap_6ln_rpl_prefix_callback, ws_bootstrap_6ln_rpl_new_parent_callback, ws_bootstrap_6ln_rpl_parent_dis_callback, cur);
...
static void ws_bootstrap_6ln_network_scan_process(protocol_interface_info_entry_t *cur)
{
...
static void ws_bootstrap_6ln_network_configuration_learn(protocol_interface_info_entry_t *cur)
{
...
static void ws_bootstrap_6ln_configure_process(protocol_interface_info_entry_t *cur)
{

    if (cur->ws_info->configuration_learned)
    {
        ws_bootstrap_6ln_network_configuration_learn(cur);
...
void ws_bootstrap_6ln_rpl_wait_process(protocol_interface_info_entry_t *cur)
{
...
void ws_bootstrap_6ln_state_machine(protocol_interface_info_entry_t *cur)
{
    ...
    case ER_ACTIVE_SCAN:
        ...
        ws_bootstrap_6ln_network_scan_process(cur);
        break;
    case ER_SCAN:
        tr_debug("WS SM:configuration Scan");
        ws_bootstrap_6ln_configure_process(cur);
        break;
    case ER_PANA_AUTH:
        tr_info("authentication start");
        // Advertisements stopped during the EAPOL
        ws_bootstrap_asynch_trickle_stop(cur);
        ws_bootstrap_6ln_fhss_configure(cur, false);
    ...
    case ER_RPL_SCAN:
        ...
        ws_bootstrap_6ln_rpl_wait_process(cur);
        break;
...
void ws_bootstrap_6ln_seconds_timer(protocol_interface_info_entry_t *cur, uint32_t seconds)
{
    ...
    if (cur->ws_info->aro_registration_timer)
    {
        ...
        else
        {
            ...
            ws_bootstrap_6ln_address_registration_update(cur, NULL);
        }
    }
